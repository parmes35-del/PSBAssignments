...
class Game:
    ...
    def status_message(self):
        return (f"{self.player1.name} HP: {self.player1.hp} | Level: {self.player1.get_level()} | XP: {self.player1.get_experience()} | "
    ...
    def attack(self):
        if self.current_attacker.is_alive():
            ...
            if not self.player1.is_alive() or not self.player2.is_alive():
                ...
                self.info_label.config(text=f"{winner.name} wins! {winner.name} is now Level {winner.get_level()} and has {winner.get_experience()} XP!")

...
class Player:
    ...
    def attack(self, opponent):
        damage = max((self.atk - opponent.def_) + random.randint(0, 5), 0)
        opponent.hp -= damage
        return damage
...
class Game:
    ...
    def attack(self):
        if self.current_attacker.is_alive():
            if self.current_attacker == self.player1:
                damage = self.player1.attack(self.player2)
                self.info_label.config(text=f"{self.player1.name} attacks {self.player2.name} for {damage} damage!")
            else:
                damage = self.player2.attack(self.player1)
                self.info_label.config(text=f"{self.player2.name} attacks {self.player1.name} for {damage} damage!")
    ...
    class Game:
        ...
        def attack(self):
            if self.current_attacker.is_alive():
                ...
                damage = self.current_attacker.attack(
                    self.player2 if self.current_attacker == self.player1 else self.player1)
                opponent_name = self.player2.name if self.current_attacker == self.player1 else self.player1.name

                self.info_label.config(
                    text=f"{self.current_attacker.name} attacks {opponent_name} for {damage} damage!")
    ...
    if __name__ == "__main__":
        ...
        if __name__ == "__main__":
                ...
                def attack(self):
                    if self.current_attacker.is_alive():
                        ...
                        if not self.player1.is_alive() or not self.player2.is_alive():
                            winner = self.player1 if self.player1.is_alive() else self.player2
                            self.info_label.config(
                                text=f"{winner.name} wins! {winner.name} gains {winner.get_experience()} XP!")
                            self.attack_button.config(state=tk.DISABLED)
                            self.restart_button.config(state=tk.NORMAL)
                        else:
                            # Switch turn to the other player
            ...
            if __name__ == "__main__":
                ...
                # Player class to handle attributes, actions, experience points, and levels
                class Player:
                    def __init__(self, name, hp, atk, def_):
                        self.name = name
                        self.hp = hp
                    ...
                    def attack(self, opponent):
                        ...
                        opponent.hp -= damage
                        if damage > 0:
                            self.gain_experience(10)  # Gain XP for hitting
                        return damage
                    ...
                    def level_up_threshold(self):
                        return 50 * self.level  # Example threshold (50 XP per level)
                ...
                class Game:
                    ...
                    def status_message(self):
                        return (
                            f"{self.player1.name} HP: {self.player1.hp} | Level: {self.player1.get_level()} | XP: {self.player1.get_experience()} | "
                            f"{self.player2.name} HP: {self.player2.hp} | Level: {self.player2.get_level()} | XP: {self.player2.get_experience()}")
                    ...
                    def attack(self):
                        if self.current_attacker.is_alive():
                            ...
                            if not self.player1.is_alive() or not self.player2.is_alive():
                                winner = self.player1 if self.player1.is_alive() else self.player2
                                self.info_label.config(
                                    text=f"{winner.name} wins! {winner.name} is now Level {winner.get_level()} and has {winner.get_experience()} XP!")
                                self.attack_button.config(state=tk.DISABLED)
                                self.restart_button.config(state=tk.NORMAL)
                            else:
                                # Switch turn to the other player
